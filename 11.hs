...
variable = mapP f (alpha &&& (digits <|> nil))
  where f (c, d) = V (c : d)
        alpha = trim (char (\c -> c >= 'a' && c <= 'z'))
        digits = trim (many1 (char isDigit))

-- Parser for lambda terms that is not ambiguous, and not left-recursive.
term, term', term'', atom :: ReadS Term
term s@(' ':xs) = term xs
term s =  term'''' s
term'''' = term''' <||> term''
term''' = mapP f (lbd &&& variable &&& sym '.' &&& term)
